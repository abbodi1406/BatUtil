...
set "lin2=%line%================"
set "_err===== ERROR ===="
set _file=(7z.dll,7z.exe,bcdedit.exe,bfi.exe,busybox.exe,cdimage.exe,esddecrypt.exe,imagex.exe,key.cmd,libwim-15.dll,offlinereg.exe,offreg.dll,wimlib-imagex.exe,wim-update.txt)
for %%# in %_file% do (
...
mkdir ISOFOLDER
echo.
"!_wimlib!" apply "!ENCRYPTEDESD!" 1 ISOFOLDER\ %_Const%
set ERRORTEMP=%ERRORLEVEL%
...
echo %line%
echo.
"!_wimlib!" export "!ENCRYPTEDESD!" 2 ISOFOLDER\sources\boot.wim --compress=maximum %_Supp%
set ERRORTEMP=%ERRORLEVEL%
...
if %WIMFILE%==install.wim set _rrr=--compress=maximum
...
:WIM
cls
if %WIMFILE%==install.wim if exist "!_workdir!\install.wim" (
echo.
echo %line%
...
"!_wimlib!" extract "!ENCRYPTEDESD!" 4 Windows\servicing\Packages\Package_for_RollupFix*.mum --dest-dir=%windir%\temp --no-acls --no-attributes %_Nul3%
...
if /i %editionid%==PPIPro set DVDLABEL=CPPIA_%archl%FRE_%langid%_DV5&set DVDISO=%_label%PPIPRO_OEM_%archl%FRE_%langid%&exit /b
if /i %editionid%==Cloud set DVDLABEL=CWCA_%archl%FRE_%langid%_DV5&set DVDISO=%_label%CLOUD_OEM_%archl%FRE_%langid%&exit /b
if /i %editionid%==CloudN set DVDLABEL=CWCNNA_%archl%FRE_%langid%_DV5&set DVDISO=%_label%CLOUDN_OEM_%archl%FRE_%langid%&exit /b
...
if /i %editionid%==ProfessionalSingleLanguage set DVDLABEL=CPRSLA_%archl%FRE_%langid%_DV5&set DVDISO=%_label%PROSINGLELANGUAGE_OEM_%archl%FRE_%langid%&exit /b
if /i %editionid%==ProfessionalCountrySpecific set DVDLABEL=CPRCHA_%archl%FRE_%langid%_DV5&set DVDISO=%_label%PROCHINA_OEM_%archl%FRE_%langid%&exit /b
exit /b
...
:GUID
(bin\busybox.exe printf "i\n" & bin\busybox.exe dd if="%ENCRYPTEDESD%" bs=1 skip=24 count=14) | bin\busybox.exe dd of=%1 bs=1 seek=24 conv=notrunc
if %2 equ 2 exit /b
...
echo %line%
echo.
for /f "tokens=3 delims=: " %%# in ('find /v /n /c "" bin\key.cmd') do set newkeys=%%#
call bin\key.cmd
if %newkeys% neq 0 for /L %%# in (1,1,%newkeys%) do (
bin\esddecrypt.exe "!ENCRYPTEDESD!" !newkey%%#! %_Nul2% && (echo Done&exit /b)
)
...
echo %line%
echo.
"!_wimlib!" export "!ENCRYPTEDESD!" 2 ISOFOLDER\!ESDarch%1!\sources\boot.wim --compress=maximum %_Supp%
set ERRORTEMP=%ERRORLEVEL%
if %ERRORTEMP% neq 0 goto :E_Export
echo.
...
popd
if %_Debug% equ 0 echo Press 0 to exit.
