...
:Begin
title Standalone Installer for Windows 8.1 ESU Updates
cd /d "!_work!"
if not exist "W81ESUI.ini" goto :proceed
findstr /i "W81ESUI-Configuration" W81ESUI.ini %_Nul1% || goto :proceed
setlocal DisableDelayedExpansion
for %%# in (
Target
Repo
...
:ReadINI
findstr /i "%1 " W81ESUI.ini >nul || goto :eof
for /f "tokens=1* delims==" %%A in ('findstr /i /c:"%1 " W81ESUI.ini') do call set "%1=%%~B"
goto :eof
...
if "!cab_dir!"=="" set "Cab_Dir=W81ESUItemp"
if "!mountdir!"=="" set "MountDir=W81ESUImount"
if "!winremount!"=="" set "WinreMount=W81ESUImountre"
...
set _drv=%~d0
if /i "%cab_dir:~0,7%"=="W81ESUI" set "cab_dir=%_drv%\W81ESUItemp"
set _ntf=NTFS
...
if /i "%mountdir:~0,7%"=="W81ESUI" set "mountdir=%_drv%\W81ESUImount"
if /i "%winremount:~0,7%"=="W81ESUI" set "winremount=%_drv%\W81ESUImountre"
...
if %offline%==1 (
dir /b /ad "!target!\Windows\servicing\Version\6.3.960*" %_Nul3% || (set "MESSAGE=Detected target offline image is not Windows NT 6.3"&goto :E_Target)
set "mountdir=!target!"
set arch=x86
...
echo ============================================================
cd /d "!targetpath!"
dism.exe /english /get-wiminfo /wimfile:"%targetname%" /index:1 | find /i "Version : 6.3.960" %_Nul1% || (set "MESSAGE=Detected wim version is not Windows NT 6.3"&goto :E_Target)
...
cd /d "!target!"
dism.exe /english /get-wiminfo /wimfile:"sources\install.wim" /index:1 | find /i "Version : 6.3.960" %_Nul1% || (set "MESSAGE=Detected install.wim version is not Windows NT 6.3"&goto :E_Target)
...
set ssuver=6.3.9600.17031
if %wimfiles% equ 0 (
for /f %%# in ('dir /b "!target!\Windows\servicing\Version"') do set "ssuver=%%#"
)
if %ssuver:~4,4% equ 9600 if %ssuver:~9,5% geq 20617 set esuready=1
if %ssuver:~4,4% geq 9601 set esuready=1
if %verbuild% equ 9600 if %svcbuild% geq 20618 set esuready=1
if %verbuild% geq 9601 set esuready=1
if %esuready% equ 0 (
if %wimfiles% equ 0 if %ssuver:~4,4% equ 9600 if %ssuver:~9,5% lss 20617 if not exist "!repo!\*Windows8.1-KB5018922*%arch%*.*" goto :E_ESU
)

:mainboard2
@cls
echo ============================================================
echo Running W81ESUI %uiv%
echo ============================================================
if %online%==1 (
...
for /f "tokens=3 delims=: " %%# in ('dism.exe /english /get-wiminfo /wimfile:"!target!\sources\boot.wim" /index:1 ^| findstr /i /c:"ServicePack Build"') do if %%# lss 20618 goto :fin
...
for /f "tokens=* delims=" %%# in ('dir /b "!repo!\*Windows8.1*%arch%*.cab"') do (set "package=%%#"&call :cab1)
)
if %_msu% neq 0 (
echo.
echo ============================================================
...
for /f "tokens=* delims=" %%# in ('dir /b "!repo!\*Windows8.1*%arch%*.msu"') do (set "package=%%#"&call :cab1)
)
...
for /f "tokens=* delims=" %%# in ('dir /b "!repo!\*Windows8.1*%arch%*.cab"') do (set "package=%%#"&set "dest=%%~n#"&call :cab2)
goto :eof
...
set _sum=0
if exist "!repo!\*Windows8.1*%arch%*.cab" (for /f "tokens=* delims=" %%# in ('dir /b "!repo!\*Windows8.1*%arch%*.cab"') do (call set /a _sum+=1))
if exist "!repo!\*Windows8.1*%arch%*.cab" (for /f "tokens=* delims=" %%# in ('dir /b "!repo!\*Windows8.1*%arch%*.cab"') do (set "package=%%#"&set "dest=%%~n#"&call :procmum))
...
findstr /i /m "WinPE" "%dest%\update.mum" %_Nul3% || (findstr /i /m "WinPE" "%dest%\Package_for_*GM~*.mum" %_Nul3% || (findstr /i /m "Package_for_RollupFix" "%dest%\update.mum" %_Nul3% || (set /a _sum-=1&goto :eof)))
)
...
findstr /i _for_KB "%dest%\update.mum" | findstr /i /v _RTM | findstr /i /v _KB2919355 | findstr /i /v _GM >>%1.xml
)
(
...
echo.
echo ==== Error ====
echo %dest% require SSU version 6.3.%ssureq% ^(at least^)
goto :eof
...
set _sum=0
cd /d "!repo!"
if exist "*Windows8.1*%arch%*.msu" (
for /f "tokens=* delims=" %%# in ('dir /b /a:-d "*Windows8.1*%arch%*.msu"') do (
  call set /a _msu+=1
  set "_name=%%#"
...
if exist "*Windows8.1*%arch%*.cab" (
for /f "tokens=* delims=" %%# in ('dir /b /a:-d "*Windows8.1*%arch%*.cab"') do (
  call set /a _cab+=1
  set "_name=%%#"
  if not "!_name!"=="!_name: =!" ren "!_name!" "!_name: =!" %_Nul3%
  if /i "!_name:~0,18!"=="AMD64_X86_ARM-all-" ren "!_name!" "!_name:~18!" %_Nul3%
...
if %%#==1 for /f "tokens=3 delims=: " %%# in ('dism.exe /english /get-wiminfo /wimfile:"!mountdir!\Windows\System32\Recovery\winre.wim" /index:1 ^| findstr /i /c:"ServicePack Build"') do if %%# lss 20618 call set winre=0
)
...
echo ==== ATTENTION ====
echo.
echo ESU updates require servicing stack update KB5018922 at least
if %online%==1 (
echo install it first and restart the system, then run the script again.
) else (
echo integrate it first, then run the script again.
)
echo.
echo Press 9 to exit.
if %_Debug% neq 0 goto :EndDebug
...
set "_pp=%_pp:"=%"
if "%_pp:~-1%"=="\" set "_pp=!_pp:~0,-1!"
if not exist "!_pp!\*Windows8.1*.msu" if not exist "!_pp!\*Windows8.1*.cab" (echo.&echo ERROR: Specified location is not valid&pause&goto :repomenu)
set "repo=!_pp!"
goto :mainmenu
