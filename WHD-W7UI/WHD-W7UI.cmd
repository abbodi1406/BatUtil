...
dism /english /get-wiminfo /wimfile:"%target%\sources\install.wim" /index:1 | find /i "Version : 6.1.7601" >nul || (set "MESSAGE=Detected install.wim version is not Windows 7 SP1"&goto :E_Target)
...
cd /d "%dest%"
if /i %arch%==x64 (set arch2=amd64) else (set arch2=x86)
set "mumfile=Package_for_!ssu!~31bf3856ad364e35~!arch2!~~6.1.2.5.mum"
...
takeown /f "%mountdir%\Windows\servicing\Packages\%mumfile%" /A 1>nul 2>nul
icacls "%mountdir%\Windows\servicing\Packages\%mumfile%" /grant *S-1-5-32-544:F 1>nul 2>nul
copy /y %mumfile% "%mountdir%\Windows\servicing\Packages\" 1>nul 2>nul
icacls "%mountdir%\Windows\servicing\Packages\%mumfile%" /reset 1>nul 2>nul
goto :eof
...
cd General\
set "dest=%cab_dir%\CRU"
if not exist "%dest%\*%rollup%*.mum" (
if %verb%==1 (
echo.
echo ============================================================
...
expand.exe -f:*Windows*.cab "*%rollup%*%arch%.msu" "%cab_dir%" >nul
md "%dest%"
expand.exe -f:* "%cab_dir%\*%rollup%*.cab" "%dest%" 1>nul 2>nul || (echo.&echo Installing %rollup%&"%dismroot%" %dismtarget% /NoRestart /Add-Package /packagepath:"%cab_dir%"&goto :eof)
)
if %verb%==1 (
echo.
echo ============================================================
...
call :cleaner
if /i %WAT% equ ON if exist "Additional\WAT\*%arch%*.msu" (expand.exe -f:*Windows*.cab Additional\WAT\*%arch%*.msu .\General >nul)
echo.
echo ============================================================
...
call :counter
call :cab
if exist "%repo%\General\*KB971033*.cab" del /f /q "%repo%\General\*KB971033*.cab" >nul
...
if /i %kb%==%ssu% (set /a _sum-=1&set /a _msu-=1&goto :eof)
if /i %kb%==KB3020369 (if exist "%mountdir%\Windows\servicing\packages\*%ssu%*.mum" set /a _sum-=1&set /a _msu-=1&goto :eof)
...
cd /d "%cab_dir%"
if /i "%cat%" equ "WMF Updates" for %%G in (2872035,2872047,2809215,3033929) do (if exist "*%%G*.cab" del /f /q "*%%G*.cab" >nul)
...
echo ============================================================
echo.
echo Installing servicing stack update %ssu%
...
if /i "%dismroot%" neq "%windir%\system32\dism.exe" (
echo.
echo ============================================================
echo Rebuilding %~nx1
echo ============================================================
...
echo 7. WMF            : %WMF%
echo 8. WAT ^(KB971033^) : %WAT%
if /i %Windows10% equ OFF (echo W. Windows10      : %Windows10%) else (echo W. Windows10      : %Windows10%        / B. Block Windows10/Telemetry: %win10u%)
echo S. ADLDS          : %ADLDS%
echo R. RSAT           : %RSAT%
if /i "%target%" neq "%SystemDrive%" (echo D. DISM : "%dismroot%")
...
echo ==================================================================
echo.
choice /c 1234567890DWSRBEIX /n /m "Change a menu option, press 0 to start, or X to exit: "
if errorlevel 18 goto :eof
if errorlevel 17 goto :indexmenu
if errorlevel 16 (if /i %winre% equ 1 (set winre=0) else (set winre=1))&goto :mainmenu
if errorlevel 15 (if /i %win10u% equ ON (set win10u=OFF) else (set win10u=ON))&goto :mainmenu
if errorlevel 14 (if /i %RSAT% equ ON (set RSAT=OFF) else (set RSAT=ON))&goto :mainmenu
if errorlevel 13 (if /i %ADLDS% equ ON (set ADLDS=OFF) else (set ADLDS=ON))&goto :mainmenu
if errorlevel 12 (if /i %Windows10% equ ON (set Windows10=OFF) else (set Windows10=ON))&goto :mainmenu
if errorlevel 11 goto :dismmenu
if errorlevel 10 goto :mainboard
if errorlevel 9 goto :countmenu
if errorlevel 8 (if /i %WAT% equ ON (set WAT=OFF) else (set WAT=ON))&goto :mainmenu
