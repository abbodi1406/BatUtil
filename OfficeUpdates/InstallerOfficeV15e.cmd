...
reg.exe query "HKU\S-1-5-19" %_Null% || (
echo %_err%
...
:office2010check
for /f "skip=2 tokens=2*" %%a in ('"reg.exe query HKLM\SOFTWARE\Microsoft\Office\14.0\Common\InstallRoot /v Path" %_Nul6%') do if exist "%%b\OSPP.VBS" (
set off2010reg=HKLM\SOFTWARE\Microsoft\Office\14.0\Common
if %arch% equ x64 (set office2010=x64&set /a _z+=1&set office2010b=64-bit {x64}) else (set office2010=x86&set /a _z+=1&set office2010b=32-bit {x86})
goto :office2013check
)
if %arch% equ x64 for /f "skip=2 tokens=2*" %%a in ('"reg.exe query HKLM\SOFTWARE\Wow6432Node\Microsoft\Office\14.0\Common\InstallRoot /v Path" %_Nul6%') do if exist "%%b\OSPP.VBS" (
...
if %arch% equ x64 (set office2013=x64&set /a _z+=1&set office2013b=64-bit {x64}) else (set office2013=x86&set /a _z+=1&set office2013b=32-bit {x86})
goto :office2016check
)
if %arch% equ x64 for /f "skip=2 tokens=2*" %%a in ('"reg.exe query HKLM\SOFTWARE\Wow6432Node\Microsoft\Office\15.0\Common\InstallRoot /v Path" %_Nul6%') do if exist "%%b\OSPP.VBS" (
...
if %arch% equ x64 (set office2016=x64&set /a _z+=1&set office2016b=64-bit {x64}) else (set office2016=x86&set /a _z+=1&set office2016b=32-bit {x86})
goto :checkdone
)
if %arch% equ x64 for /f "skip=2 tokens=2*" %%a in ('"reg.exe query HKLM\SOFTWARE\Wow6432Node\Microsoft\Office\16.0\Common\InstallRoot /v Path" %_Nul6%') do if exist "%%b\OSPP.VBS" (
...
for /f "delims=" %%G in ('dir /b /s *2010*%office2010%*.msp %_Nul6%') do (echo %%G>>"!_temp!\update1.burf")
...
set /a sp=sp2010+sp2013 %_Nul1%
if %sp% neq 3 (goto :spinstall)

if not defined _c (goto :nofiles)

pushd "!_temp!"
reg.exe query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Installer\UserData\S-1-5-18\Products" /f "(KB" /s /d 1>"installed.burf" %_Nul2%
...
popd
set /a _c=C2010+C2013+C2016 %_Nul1%
...
expand.exe -f:*.msp "%installer%" .\ %_Nul1%
...
del /f /q "!_temp!\*_MSPLOG.LOG" %_Nul3%
...
set /a spinst=spinst2010+spinst2013 %_Nul1%
