...
for /f %%G in ('type "filelist1.burf"') do (call set /a c+=1)
popd
)
...
for /f %%G in ('type "filelist2.burf"') do (call set /a c+=1)
popd
)
...
pushd "!_temp!"
type nul>"update3a.burf"
findstr /i /v "superseded" "update3a.burf" >"filelist3.burf"
for /f %%G in ('type "filelist3.burf"') do (call set /a c+=1)
popd
)
